...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($user) {
        if ($password == $user['password']) {
            ...
            // Standardizing globally recognized session keys
...
// Header is now safely included down here after all redirect blocks execute
...
<div class="max-w-4xl mx-auto bg-white rounded-3xl border border-sky-100/60 shadow-xl overflow-hidden min-h-[500px] flex flex-col md:flex-row mt-4">
    ...
    <div class="md:w-1/2 bg-sky-50 relative flex flex-col justify-between p-8 text-center border-b md:border-b-0 md:border-r border-sky-100/40">
        ...
        <div class="relative z-10 my-auto flex flex-col items-center">
            <img src="/PawTrack/assets/images/Cat Loading.jpg" alt="PawTrack Shelter Art" class="w-72 h-auto object-contain rounded-2xl mix-blend-multiply transition duration-500 hover:scale-102">
    ...
    <div class="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
        ...
        <form method="POST" class="space-y-4">
            ...
            <button type="submit" class="w-full bg-sky-500 hover:bg-sky-600 text-white font-semibold py-3.5 rounded-xl shadow-md hover:shadow-lg transition duration-200 text-sm mt-2">
                Sign In
            </button>
        </form>

        <p class="mt-8 text-center text-sm text-slate-500">
            Don't have an account? <a href="/PawTrack/auth/register.php" class="text-sky-600 font-semibold hover:underline">Register here</a>
        </p>
    </div>
</div>
